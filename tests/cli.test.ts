...
describe('parseArgs', () => {
  ...
  test('num rejects a non-numeric value rather than yielding NaN', () => {
...
describe('dispatch', () => {
  ...
  test('a designed-but-unbuilt command fails loudly with NOT_IMPLEMENTED', () => {
    const e = dispatch(['search', 'people', 'x'], T0);
  ...
  test('doctor reports rather than throwing when nothing is set up', () => {
    const e = dispatch(['doctor', '--offline'], T0);
    expect(e.ok).toBe(true);
    expect(exitCodeFor(e)).toBe(0);
  });

  test('doctor is honest that the engine does not exist yet', () => {
    const e = dispatch(['doctor', '--offline'], T0);
  ...
  test('budget reports every spend class with its cap provenance', () => {
    const e = dispatch(['budget'], T0);
    if (!e.ok) throw new Error('expected ok');
  ...
  test('budget carries the caveat that the numbers are not measured limits', () => {
    const e = dispatch(['budget'], T0);
    if (!e.ok) throw new Error('expected ok');
    expect((e.data as { caveat: string }).caveat).toContain('no corroborated');
  });

  test('clearing a cooldown without --confirm is refused', () => {
    const e = dispatch(['budget', '--reset-cooldown'], T0);
    if (e.ok) throw new Error('expected refusal');
    expect(e.error.code).toBe('CONFIRMATION_REQUIRED');
  });

  test('risk reports a clear breaker when nothing has gone wrong', () => {
    const e = dispatch(['risk'], T0);
    if (!e.ok) throw new Error('expected ok');
    expect((e.data as { state: string }).state).toBe('ok');
  });

  test('risk states the ToS breach plainly rather than claiming compliance', () => {
    const e = dispatch(['risk'], T0);
    if (!e.ok) throw new Error('expected ok');
  ...
  test('a corrupt ledger fails loudly instead of restoring a full budget', () => {
